...
```cpp
if (ImGui::BeginTable("process_data", 4,
...
```cpp
if (ImGui::BeginTable("advanced", 3,
...
if (ImPlot::BeginPlot("Real-time Temperature", ImVec2(-1, 300))) {
    ...
    ImPlot::SetupAxisLimits(ImAxis_X1,
        time_counter - 10, time_counter,
        ImGuiCond_Always
    );
    ImPlot::SetupAxisLimits(ImAxis_Y1, 400, 500);

    ImPlot::PlotLine("Temperature",
        &temp_buffer.data()[0].x,
        &temp_buffer.data()[0].y,
        temp_buffer.size(),
        0,
        temp_buffer.offset(),
        2 * sizeof(float)
...
```cpp
void render_gauge(const char* label, float value,
    ...
    draw_list->AddCircle(center, radius,
        IM_COL32(100, 100, 100, 255), 64, 2.0f);
    ...
    draw_list->AddLine(center, ImVec2(needle_x, needle_y),
        IM_COL32(255, 0, 0, 255), 3.0f);

    // 중심점
    draw_list->AddCircleFilled(center, 5.0f,
        IM_COL32(255, 0, 0, 255));
    ...
    draw_list->AddText(
        ImVec2(center.x - text_size.x / 2, center.y + radius - 30),
        IM_COL32(255, 255, 255, 255),
        value_text
    );
...
// 렌더링
ImGui::Image(
    (void*)(intptr_t)equipment_image.id,
    ImVec2(equipment_image.width, equipment_image.height)
);
...
// 기본 열림
if (ImGui::CollapsingHeader("Alarms",
...
private:
    ...
    void render_chart_panel() {
        ...
        if (ImPlot::BeginPlot("Temperature", ImVec2(-1, 200))) {
            ImPlot::SetupAxes("Time (s)", "Temperature (°C)");
            ImPlot::SetupAxisLimits(ImAxis_X1,
                time_ - 10, time_, ImGuiCond_Always);
            ImPlot::SetupAxisLimits(ImAxis_Y1, 400, 500);

            ImPlot::PlotLine("Temperature",
                &temp_buffer_->data()[0].x,
                &temp_buffer_->data()[0].y,
                temp_buffer_->size(),
                0,
                temp_buffer_->offset(),
                2 * sizeof(float)
        ...
        if (ImPlot::BeginPlot("Pressure", ImVec2(-1, 200))) {
            ...
            ImPlot::PlotLine("Pressure",
                &pressure_buffer_->data()[0].x,
                &pressure_buffer_->data()[0].y,
                pressure_buffer_->size(),
                0,
                pressure_buffer_->offset(),
                2 * sizeof(float)
            );
