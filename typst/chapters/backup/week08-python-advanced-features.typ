...
class QtEquipmentClient(QObject):
...
class EquipmentOPCServer:
    def __init__(self):
        ...
        # Variables
        self.temp_var = self.equipment.add_variable(
            self.idx, "Temperature", 0.0
        )
        self.temp_var.set_writable()

        self.pressure_var = self.equipment.add_variable(
    ...
    def update_temperature(self, value: float):
        self.temp_var.set_value(value)

    def update_pressure(self, value: float):
        self.pressure_var.set_value(value)
```
...
class EquipmentOPCClient:
    ...
    def write_temperature(self, value: float):
        node = self.client.get_node("ns=2;s=Equipment.Temperature")
...
class ModbusEquipmentClient:
    ...
    def read_temperature(self) -> float:
        ...
        if not result.isError():
            return result.registers[0] / 10.0  # 스케일링
    ...
    def write_setpoint(self, value: float):
        ...
        self.client.write_register(
            address=100, value=scaled_value, slave=1
        )

    def read_multiple(self, addresses: list[int]) -> list[float]:
        result = self.client.read_holding_registers(
            address=addresses[0],
            count=len(addresses),
            slave=1
        ...
        if not result.isError():
            return [r / 10.0 for r in result.registers]
        raise Exception("Read error")
```
...
class QtModbusClient(QObject):
    ...
    def poll_data(self):
        try:
            temp = self.client.read_temperature()
...
class DatabaseManager:
    ...
    def insert_data(self, data: ProcessData, equipment_id: str):
        with self.get_connection() as conn:
            conn.execute("""
    ...
    def query_data(self, equipment_id: str,
                   start_time: datetime,
                   end_time: datetime) -> list[ProcessData]:
        with self.get_connection() as conn:
            cursor = conn.execute("""
...
```python
class IntegratedHMI(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    @Slot(str)
    def on_tcp_data(self, data: str):
        # TCP 데이터 처리
        print(f"TCP Data: {data}")

    @Slot(float)
    def on_modbus_data(self, value: float):
        # Modbus 데이터 처리 및 저장
        data = ProcessData(
